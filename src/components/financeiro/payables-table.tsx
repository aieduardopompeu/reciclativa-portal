...
function RowActions({
  ...
  partialPayAction: (formData: FormData) => void | Promise<void>;
  ...
  if (item.status === "paid") {
    return (
      <div className="space-y-2 lg:max-w-[280px]">
        <p className="text-xs font-semibold text-emerald-700">Baixada</p>
        <form
          ...
          onSubmit={(event) => {
            ...
            if (!reason) return;
            if (!window.confirm("Confirmar estorno desta conta paga?")) {
              event.preventDefault();
            }
          }}
        >
          <input type="hidden" name="payable_id" value={item.id} />
          <input
            type="text"
            name="reverse_reason"
            required
  ...
  return (
    <div className="grid gap-3 xl:grid-cols-2 xl:items-start">
      <form
        action={payAction}
        onSubmit={(event) => {
          if (!window.confirm(`Confirmar quitação total desta conta no valor de ${formatMoney(remaining)}?`)) {
            event.preventDefault();
          }
        }}
      ...
      <form
        ...
        onSubmit={(event) => {
          ...
          if (!raw) return;
          if (!window.confirm(`Confirmar baixa parcial de ${formatMoney(Number(raw))} nesta conta?`)) {
            event.preventDefault();
          }
        }}
      >
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-emerald-100 bg-emerald-50/70 px-4 py-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="text-sm text-emerald-800">Selecione contas em aberto ou parciais para quitar tudo de uma vez.</p>
        ...
        <div className="flex flex-wrap items-center gap-2 lg:justify-end">
          ...
          <form
            ...
            onSubmit={(event) => {
              ...
              if (!window.confirm(`Confirmar quitação em lote de ${selectedCount} conta(s) selecionada(s)?`)) {
                event.preventDefault();
              }
            }}
          >
            {selectedIds.map((id) => <input key={id} type="hidden" name="payable_ids" value={id} />)}
            <ActionButton
              idleLabel={`Quitar selecionadas (${selectedCount})`}
            ...
            <div key={item.id} className={`rounded-2xl border border-black/10 bg-white p-4 shadow-sm ${isSelected ? "ring-2 ring-emerald-200" : ""}`}>
              <div className="mb-3 flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs text-slate-600">{item.supplier_name || "Sem fornecedor"} • {item.unit_name}</p>
                </div>
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                ...
                <div className="rounded-xl bg-slate-50 p-3"><div className="text-slate-500">Valor pago</div><div className="mt-1 font-semibold text-slate-900">{formatMoney(paid)}</div></div>
              ...
              <div className="flex items-start gap-4 px-4 py-4 text-sm text-slate-700 xl:items-center">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-start justify-between gap-4 xl:hidden">
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-500">
                        <span>{item.supplier_name || "Sem fornecedor"}</span>
                        <span>•</span>
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-2 xl:grid-cols-4">
                    ...
                    <div className="rounded-xl bg-slate-50 px-3 py-2">
                      <div className="text-[11px] uppercase tracking-[0.12em] text-slate-400">Pago</div>
                      <div className="mt-1 font-semibold text-slate-900">{formatMoney(paid)}</div>
                    </div>
                    <div className="rounded-xl bg-slate-50 px-3 py-2">
                ...
                <div className="border-t border-black/10 bg-slate-50/70 px-4 py-3">
                  <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr),380px] xl:items-start">
                    <div className="grid grid-cols-2 gap-2 text-xs text-slate-600">
                      <div className="rounded-xl border border-black/5 bg-white px-3 py-2"><span className="block text-[11px] uppercase tracking-[0.08em] text-slate-400">Fornecedor</span><span className="mt-1 block font-medium text-slate-800">{item.supplier_name || "Sem fornecedor"}</span></div>
                    ...
                    <div className="rounded-2xl border border-black/5 bg-white p-3">
                      <RowActions
                        item={item}
                        payAction={payAction}
                        partialPayAction={partialPayAction}
