...
async function getPayables(organizationId: string): Promise<PayableRow[]> {
...
function EmptyState() {
  return (
    <div className="rounded-2xl border border-dashed border-black/15 bg-slate-50 px-5 py-8 text-sm text-slate-600">
      Nenhuma conta a pagar encontrada para esta organização.
    </div>
  );
}
...
export default async function PayablesPage() {
  ...
  const [payables, units, suppliers, financialAccounts] = await Promise.all([
    getPayables(user.organization.id),
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-slate-600">
              Primeira base real do módulo financeiro para registrar e baixar obrigações da empresa.
            </p>
          </div>
        ...
        <form action={createPayableAction} className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            ...
            <label className="block">
              ...
              <select name="supplier_id" defaultValue="" className="w-full rounded-2xl border border-black/10 bg-slate-50 px-4 py-2.5 text-sm outline-none">
            ...
            <label className="block xl:col-span-2">
              ...
              <input name="description" required className="w-full rounded-2xl border border-black/10 bg-slate-50 px-4 py-2.5 text-sm outline-none" placeholder="Ex.: Frete do mês" />
            </label>

            <label className="block">
              <span className="mb-1 block text-sm font-medium text-slate-700">Documento</span>
              <input name="document_number" className="w-full rounded-2xl border border-black/10 bg-slate-50 px-4 py-2.5 text-sm outline-none" placeholder="NF / boleto / ref." />
            </label>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <p className="text-sm text-slate-500">O registro será salvo em <code>payables</code> e auditado em <code>audit_logs</code>.</p>
          </div>
        </form>
      </section>

      <section className="rounded-3xl border border-black/10 bg-white p-6 shadow-sm">
        {payables.length === 0 ? <EmptyState /> : (
                    ...
                    <td className="px-4 py-3">
                      {item.status === 'paid' ? (
                        ...
                        <form action={payPayableAction}>
                          ...
                          <button type="submit" className="rounded-xl bg-emerald-600 px-3 py-2 text-xs font-semibold text-white transition hover:bg-emerald-700">Baixar</button>
                        </form>
                      )}
                    </td>
                  </tr>
