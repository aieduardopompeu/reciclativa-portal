...
import { getAllTags, getPostsByTag } from "@/content/blog/posts";
...
export default async function TagPage({ params }: Props) {
  ...
  const posts = getPostsByTag(decoded).sort((a, b) =>
    a.dateISO < b.dateISO ? 1 : -1
  );

  const base = siteUrl();
  const canonicalUrl = `${base}/blog/tags/${decoded}`;
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Tags", item: `${base}/blog` },
      { "@type": "ListItem", position: 4, name: label, item: canonicalUrl },
    ],
  };

  // Chips dinâmicos: “Todos” + todas as tags encontradas em posts
      ...
      <header className="relative overflow-hidden border-b border-slate-200">
        ...
        <div className="relative mx-auto w-full max-w-6xl px-4 py-12 sm:px-6 lg:px-8">
          ...
          <p className="mt-4 max-w-2xl text-base leading-relaxed text-slate-700">
            Veja os artigos relacionados a <strong>{label}</strong>.
          </p>
        ...
        {posts.length === 0 ? (
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <p className="mt-2 text-sm text-slate-700">
              Ainda não temos conteúdos para esta tag.
            </p>
          </div>
        ) : (
          <>
            <div className="mb-6 flex items-center justify-between gap-3">
              <p className="text-sm text-slate-600">
                {posts.length} {posts.length === 1 ? "artigo" : "artigos"} nesta tag
              </p>
              <Link
                href="/blog"
