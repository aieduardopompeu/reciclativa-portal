...
export async function GET(
  ...
  const found = await sql<{ id: number; status: string; relevancia: number; destaque: boolean }>`
    select id, status, relevancia, destaque
    from radar_noticias
  ...
  redirectUrl.searchParams.set("aprovado", "1");
  if (row.relevancia === 5 && !row.destaque) {
    redirectUrl.searchParams.set("sugerir_destaque", "1");
  }
